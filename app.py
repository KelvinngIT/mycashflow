...
# ======================
st.set_page_config(
    page_title="Excel Currency Converter",
    page_icon="💱",
    layout="wide"
)
...
def process_dataframe(df: pd.DataFrame, target_currency: str):
    ...
    col_map = {
        "company": ["company", "company name", "comp", "entity"],
        "party": ["party name", "party", "customer name", "customer", "payee name", "payee", "vendor", "supplier"],
    ...
    result = pd.DataFrame()
    result["Company"] = df[found["company"]] if "company" in found else ""
    result["Party Name"] = df[found["party"]] if "party" in found else ""
    result["Currency"] = df[found["currency"]].apply(normalize_currency)
    result["Amount"] = pd.to_numeric(df[found["amount"]], errors="coerce")
    result["Payment Date"] = pd.to_datetime(df[found["payment_date"]], errors="coerce") if "payment_date" in found else pd.NaT
    result["Category"] = df[found["category"]].astype(str).str.strip() if "category" in found else ""
    ...
    unsupported = result[result[converted_col].isna() & result["Amount"].notna()]["Currency"].unique()
    if len(unsupported) > 0:
        warnings.append(f"Unsupported currencies (left blank): {', '.join(map(str, unsupported))}")

    # Flag non-standard categories
    non_standard = result[~result["Category"].isin(ALL_STANDARD_CATEGORIES) & (result["Category"] != "") & (result["Category"].str.lower() != "nan")]
    if len(non_standard) > 0:
        unique_non_std = non_standard["Category"].unique()
        warnings.append(
            f"Non-standard categories found ({len(non_standard)} rows): "
            f"{', '.join(map(str, unique_non_std[:8]))}"
            + ("..." if len(unique_non_std) > 8 else "")
        )
...
# ======================
if not st.session_state.logged_in:
    st.title("💱 Excel Currency Converter")
...
st.markdown("""
Upload an Excel or CSV file containing transaction data.  
The app will standardize the columns and convert all amounts into your chosen currency (**HKD / SGD / USD**).
""")
...
uploaded_file = st.file_uploader(
    ...
    type=["xlsx", "xls", "csv"],
    help="Expected columns: Company, Party Name, Currency, Amount, Payment Date, Category"
)
...
if uploaded_file is not None:
    try:
        ...
        st.caption("Current approximate rates (via USD): HKD ≈ 7.80, SGD ≈ 1.32, USD = 1.00")
        ...
        if st.button("🔄 Process & Convert", type="primary", use_container_width=True):
            ...
            if result_df is None:
                ...
                default_name = f"converted_{target_currency}_{timestamp}.xlsx"
...
• Recommended categories are shown above – non-standard ones are flagged but still kept  
• Exchange rates are approximate (update `RATES_TO_USD` for production use)  
